...
def main():
    ...
    pred_file = json.load(open(args.pred_file, "r"))
    ans_file = json.load(
        open("/home/jihyunlee/dstc11-track5/data/val/labels.json", "r")
    )
    knowledge = eval_dataset.snippets
    res = []
    for ans_dict, pred, dial in zip(ans_file, pred_file, eval_dataset.dialogs):
        temp = {}
        user = dial["log"][-1]["text"]
        temp["user"] = user
        ans_keys, pred_keys = [], []

        if "knowledge" not in ans_dict and "knowledge" not in pred:
            continue
        if "knowledge" in ans_dict:
            ans_keys = [get_key(snippet) for snippet in ans_dict["knowledge"]]
        if "knowledge" in pred:
            pred_keys = [get_key(snippet) for snippet in pred["knowledge"]]

        FN = set(ans_keys) - set(pred_keys)
        FP = set(pred_keys) - set(ans_keys)

        temp["FN"] = []
        temp["FP"] = []

        for fn in list(FN):
            know_text = tokenizer.decode(knowledge[fn]["token_ids"])
            temp["FN"].append(know_text)
        for fp in list(FP):
            know_text = tokenizer.decode(knowledge[fp]["token_ids"])
            temp["FP"].append(know_text)
        res.append(temp)
