...
do
    ...
    model_name_exp=deberta-v3-base-typo-${i}
    cuda_id=0,1

    CUDA_VISIBLE_DEVICES=${cuda_id} python baseline.py \
            --task selection \
            --dataroot Typo_repeat/repeats/${i} \
...
do
    ...
    rg_output_file=pred/${eval_dataset}/baseline.rg.${model_name_exp}-typo-${i}.json
    ...
    rg_output_score_file=pred/${eval_dataset}/baseline.rg.${model_name_exp}-typo-${i}.json
