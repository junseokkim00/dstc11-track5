method=fus_randInsert
model_name=microsoft/deberta-v3-base
    model_name_exp=deberta-v3-base-${method}
    cuda_id=0,1

    CUDA_VISIBLE_DEVICES=${cuda_id} python baseline.py \
            --task selection \
            --dataroot fusAndrandInsert \
...
rg_output_file=pred/${eval_dataset}/baseline.rg.${model_name_exp}-${method}.json
...
rg_output_score_file=pred/${eval_dataset}/baseline.rg.${model_name_exp}-${method}.json
